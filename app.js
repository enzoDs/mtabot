...
for (let botconfig of config.bots) {
    ...
    bot.on("ready", () => {
        console.log(`Bot ${bot.name} conectado`);
        channelBots.set(channel, bot);
    ...
    bot.on("disconnect", () => {
        console.log(`Bot ${bot.name} desconectado`);
    ...
    bot.on("session.bind", (session) => {
        if (!session.get("bind-message")) {
            bot.sendMessage(`**Bot:** Se establecio conexion con el server (by enzoDs) :white_check_mark: `);
    ...
    function login() {
        bot.login()
            .catch((error) => {
                console.error(`El bot fallo al logearse: ${error.message}`);
